...
namespace TGS {
    ...
    public partial class TerrainGridSystem : MonoBehaviour {
        ...
        void DisplayCellInfo() {
            ...
            if (_displayCellDebugInfo == CellDebugInfo.Nothing) return;

            if (_displayCellDebugInfo == CellDebugInfo.CellCoordinates && _gridTopology == GridTopology.Irregular) return;

            int cellsCount = cells.Count;
            cellsCount = Mathf.Min(cellsCount, 2000);
            GUIStyle style = new GUIStyle(GUI.skin.label);
            style.alignment = TextAnchor.MiddleCenter;
            if (cellDebugInfoLabels == null || cellDebugInfoLabels.Length != cellsCount || currentCellDebugInfo != _displayCellDebugInfo) {
                currentCellDebugInfo = _displayCellDebugInfo;
                cellDebugInfoLabels = new string[cellsCount];
                switch (_displayCellDebugInfo) {
        ...
        void UpdateMaterialDepthOffset() {
            ...
            territoriesGradientMat.SetFloat(ShaderParams.Offset, depthOffset);
        ...
        void DrawInteriorTerritoryFrontiers() {
            ...
            for (int k = 0; k < territories.Count; k++) {
                Territory territory = territories[k];
                TerritoryDrawInteriorBorder(k, _territoryInteriorBorderPadding, _territoryInteriorBorderThickness, includeEnclaves: _territoryInteriorBorderIncludeEnclaves);
            }
        }
        ...
        GameObject GenerateRegionSurface(Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace, CELL_TEXTURE_MODE textureMode) {
                ...
                if (!hasHole) {
                    if (_gridTopology == GridTopology.Hexagonal && pointCount == 6) {
                        if (textureMode == CELL_TEXTURE_MODE.InTile) {
                            ...
                            return GenerateRegionSurfaceFloatingQuad(region, cacheIndex, material, textureScale, textureOffset, textureRotation, rotateInLocalSpace);
                        }
        ...
        GameObject GenerateRegionSurfaceHighlightHex(Material material) {
            ...
            Rect rect = (canvasTexture != null && material != null && material.mainTexture == canvasTexture) ? canvasRect : region.rect2D;
            Renderer surfRenderer = Drawing.CreateSurface("Highlight Hex", meshPoints, hexIndices, material, rect, Misc.Vector2one, Misc.Vector2zero, 0, false, _sortingOrder, disposalManager);
            GameObject surf = surfRenderer.gameObject;
            surf.transform.SetParent(surfacesLayer.transform, false);
            surf.transform.localPosition = Misc.Vector3zero;
            surf.layer = gameObject.layer;
        ...
        GameObject GenerateRegionSurfaceFloatingQuad(Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
        ...
        GameObject GenerateTerritoryRegionSurface(int territoryIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace, int regionIndex = 0) {
            ...
            int cacheIndex = GetCacheIndexForTerritoryRegion(territoryIndex, regionIndex);
            return GenerateRegionSurface(region, cacheIndex, material, textureScale, textureOffset, textureRotation, rotateInLocalSpace, CELL_TEXTURE_MODE.InTile);
        }
        ...
        public GameObject HighlightCellRegion(int cellIndex, bool refreshGeometry) {
            ...
            if (_highlightEffect != HighlightEffect.None) {
                ...
                if (_terrainWrapper == null && _gridTopology != GridTopology.Irregular) {
                    if (_highlightedRefSurface == null) {
                        _highlightedRefSurface = GenerateRegionSurfaceHighlightHex(hudMatCell);
                    }
                    Vector2 cellCenter = cell.scaledCenter;
                    Vector2 cellCenterRef = cells[0].scaledCenter;
                    ...
                    if (existsInCache) {
                        ...
                        _highlightedObj = GenerateCellRegionSurface(cellIndex, hudMatCell, Misc.Vector2one, Misc.Vector2zero, 0, false, CELL_TEXTURE_MODE.InTile);
                    }
                }
        ...
        GameObject GenerateCellRegionSurface(int cellIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace, CELL_TEXTURE_MODE textureMode) {
            if (!ValidCellIndex(cellIndex)) return null;
            Region region = cells[cellIndex].region;
            int cacheIndex = GetCacheIndexForCellRegion(cellIndex);
            return GenerateRegionSurface(region, cacheIndex, material, textureScale, textureOffset, textureRotation, rotateInLocalSpace, textureMode);
        }
        ...
        void CellAnimate(FaderStyle style, int cellIndex, Color initialColor, Color color, float duration, int repetitions) {
            ...
            MeshFilter mf = animatedSurface.GetComponent<MeshFilter>();
            mf.sharedMesh = cellSurface.GetComponent<MeshFilter>().sharedMesh;
            Material fadeMaterial = materialPool.Get(fadeMat);
